...
Face_Get_Advances_Func :: #type proc "c" (face: Face, first, count: c.uint, flags: i32, advances: ^c.long) -> Error
...
B_Box :: struct {
    x_min, y_min, x_max, y_max : c.long,
}
...
Glyph_Metrics :: struct {
    ...
    hori_bearing_x : c.long,
    hori_bearing_y : c.long,
    hori_advance   : c.long,

    vert_bearing_x : c.long,
    vert_bearing_y : c.long,
    vert_advance   : c.long,
}
...
Glyph_Slot_Rec :: struct {
    ...
    linear_hori_advance : c.long,
    linear_vert_advance : c.long,
    advance             : Vector,
...
Module_Class :: struct {
    ...
    module_size      : c.long,
    module_name      : cstring,
    module_version   : c.long,
    module_requires  : c.long,
...
round :: #force_inline proc(#any_int x: int) -> int {
    if x >= 0 do return (x + 0x1f) >> 6
    return -(((-x) + 0x1f) >> 6)
}
